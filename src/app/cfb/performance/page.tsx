            ...
            {rows.map((m) => (
              <TableRow key={`${m.segment_kind}-${m.segment}`}>
                <TableCell className="font-medium">
                  ...
                  {m.thin_sample && (
                    <span className="ml-2 text-xs text-muted-foreground">thin</span>
                  )}
                </TableCell>
                <TableCell className={numCell}>{m.games ?? "–"}</TableCell>
                <TableCell className={numCell}>{formatNumber(m.margin_mae, 2)}</TableCell>
                <TableCell className={numCell}>{formatNumber(m.market_mae, 2)}</TableCell>
...
export default async function PerformancePage() {
  ...
  try {
    [backtest, live] = await Promise.all([fetchFullBacktest(), fetchLivePerformance()]);
  } catch {
  ...
  return (
    <main id="main" className="mx-auto w-full max-w-5xl min-w-0 px-4 py-8 space-y-10">
          ...
          <>
            {livePure.thin_sample && (
              <p className="rounded-md border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
                {livePure.games} games graded through {fmtDate(live.latestKickoff)}.
                Fewer than 30 games describe the season so far, not model
                skill. Read these as status, not evidence.
              </p>
            )}
      ...
      <section className="space-y-6">
        <div className="flex items-start justify-between gap-4">
          <h2 className="font-heading text-lg">Historical walk-forward backtest</h2>
          {seasons.length > 0 && (
        ...
        {!overall ? (
          <p className="text-sm text-muted-foreground">No backtest data published yet.</p>
        ) : (
          <>
            <BacktestKpis
              overall={overall}
